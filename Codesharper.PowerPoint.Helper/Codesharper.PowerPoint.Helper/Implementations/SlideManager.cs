...
namespace Codesharper.PowerPoint.Helper.Implementations
{
    ...
    public class SlideManager : ISlideManager
    {
        ...
        public void Export(PPT.Slide slide, string filePathAndName, ImageFormats.Formats exportFormat, int scaleWidth = 1280, int scaleHeight = 1024)
        {
       ...
       /// <param name="filePath">File Path to export all the slides to (excluding filename(s))</param>
        ...
        /// <param name="scaleWidth">Scale with of the image</param>
        /// <param name="scaleHeight">Scale height of the image</param>
        public void ExportAll(PPT.Presentation presentation, string filePath, ImageFormats.Formats exportFormat, int scaleWidth = 1280, int scaleHeight = 1024)
        {
            foreach (PPT.Slide slide in presentation.Slides)
            {
                slide.Export(filePath + @"slide" + slide.SlideIndex.ToString() + "." + exportFormat.ToString(), exportFormat.ToString(), scaleWidth, scaleHeight);
            }
        }
    }
}
